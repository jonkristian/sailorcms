<script lang="ts">
  ...
  async function bulkRestore(ids: string[]) {
    ...
    const results = await Promise.all(
      ids.map((id) =>
        restoreGlobalItem({ globalSlug: data.slug, itemId: id }).catch(() => ({
          success: false,
    ...
    try {
      const results = await Promise.all(
        pendingPurge.items.map(async (it) => {
          try {
            return await purgeGlobalItem({
              globalSlug: data.slug,
              itemId: it.id
            });
          } catch {
...
<div class="container mx-auto px-6">
  ...
  <Header
    title={data.label}
    description="Soft-deleted items. Restore to return them, or permanently delete."
