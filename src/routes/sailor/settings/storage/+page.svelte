  ...
  <div class="flex flex-col gap-6">
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>Configure storage settings using environment variables</Card.Description>
      </Card.Header>
      <Card.Content>
        <div class="grid grid-cols-2 gap-6">
          <div>
            <h4 class="text-muted-foreground mb-2 text-sm font-medium">Storage Provider</h4>
            <code class="bg-muted rounded px-2 py-1 text-xs">STORAGE_PROVIDER=local|s3</code>
          </div>

          <div>
            <h4 class="text-muted-foreground mb-2 text-sm font-medium">Local Storage</h4>
            <div class="space-y-1">
              <code class="bg-muted rounded px-2 py-1 text-xs">UPLOAD_DIR=static/uploads</code><br
              />
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >(PUBLIC_URL derived from UPLOAD_DIR)</code
              >
            </div>
          </div>

          <div>
            <h4 class="text-muted-foreground mb-2 text-sm font-medium">S3 Storage</h4>
            <div class="space-y-1">
              <code class="bg-muted rounded px-2 py-1 text-xs">S3_BUCKET=your-bucket</code><br />
              <code class="bg-muted rounded px-2 py-1 text-xs">S3_REGION=us-east-1</code><br />
              <code class="bg-muted rounded px-2 py-1 text-xs">S3_ACCESS_KEY_ID=your-key</code><br
              />
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >S3_SECRET_ACCESS_KEY=your-secret</code
              ><br />
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >S3_ENDPOINT=https://s3.amazonaws.com</code
              ><br />
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >S3_PUBLIC_URL=https://your-bucket.s3.amazonaws.com</code
              >
            </div>
          </div>

          <div>
            <h4 class="text-muted-foreground mb-2 text-sm font-medium">Upload Settings</h4>
            <div class="space-y-1">
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >UPLOAD_FOLDER_STRUCTURE=flat|date|type</code
              ><br />
              <code class="bg-muted rounded px-2 py-1 text-xs">UPLOAD_MAX_FILE_SIZE=10.0MB</code><br
              />
              <code class="bg-muted rounded px-2 py-1 text-xs"
                >UPLOAD_ALLOWED_TYPES=image/*,application/pdf</code
              >
            </div>
          </div>

          <div>
            <h4 class="text-muted-foreground mb-2 text-sm font-medium">Cache Settings</h4>
            <div class="space-y-1">
              <code class="bg-muted rounded px-2 py-1 text-xs">CACHE_PROVIDER=local|disabled</code
              ><br />
              <code class="bg-muted rounded px-2 py-1 text-xs">CACHE_DIR=static/cache</code><br />
              <code class="bg-muted rounded px-2 py-1 text-xs">CACHE_MAX_SIZE=1GB</code>
            </div>
          </div>
        </div>
      </Card.Content>
