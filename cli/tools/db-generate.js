...
async function generateSchema() {
  ...
  // Opt-in: block grouping turns on when `blocks.groups` is declared in
  // settings.ts (and not explicitly disabled). Omit the block → feature off
  // (no block_groups table, no editor UI). Single source of truth, no surprise.
  const groupSettings = settings?.blocks?.groups;
  const groupsEnabled = groupSettings != null && groupSettings.enabled !== false;
  const groupFields = groupsEnabled ? (groupSettings.fields ?? {}) : {};
  ...
  const generator = new SchemaGenerator(
    adapter,
    { globalDefinitions, collectionDefinitions, blockDefinitions, groupFields, groupsEnabled },
...
/**
 * Emit `generated/block-groups.ts` with the block-group setting field
 * definitions, so the admin UI (BlockGroupContainer) can render them via
 * FieldRenderer and the persist/load helpers can derive the column list.
 */
function generateBlockGroups(targetDir, { groupFields, groupsEnabled }) {
  const generatedDir = path.join(targetDir, 'src/lib/sailor/generated');
  if (!fs.existsSync(generatedDir)) fs.mkdirSync(generatedDir, { recursive: true });
  const content = `// AUTO-GENERATED by sailor db:update — do not edit by hand.
// Block group settings (from settings.ts blocks.groups, or core defaults).
export const blockGroupsEnabled = ${groupsEnabled ? 'true' : 'false'};

export const blockGroupFields = ${JSON.stringify(groupFields, null, 2)} as Record<string, any>;

export const blockGroupColumns = Object.keys(blockGroupFields);
`;
  fs.writeFileSync(path.join(generatedDir, 'block-groups.ts'), content);
}
