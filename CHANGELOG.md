...
- **Path B scaled from one component to the entire `components/sailor/` tree.** All 64 admin components now resolve from the `sailorcms` package instead of being copied to the consumer. Skip mechanism in `cms-init` / `cms-update` switched from per-file to per-directory (`COMPONENT_DIRS_RESOLVED_VIA_PACKAGE = ['sailor']`), so adding new components in that subtree requires no further bookkeeping. 45 import sites rewritten from `$lib/components/sailor/...` to `sailorcms/components/sailor/...`; ~16 barrel imports (`{ X } from '.../dnd'`, `{ Y } from '.../table'`, etc.) replaced with file-specific paths so the exports map stays at four wildcard lines instead of needing to enumerate every barrel directory. Added `@source "../../../../node_modules/sailorcms/src/lib"` to `sailor.css` so Tailwind v4 scans the migrated package source for class usage — without it the default scan misses `node_modules` and admin styles render partially (header padding, badge variants, etc. silently absent).

- **Path B migrated `sailor/styles/` next.** `sailor.css` now resolves from `sailorcms/styles/sailor.css` instead of being copied to the consumer's `src/lib/sailor/styles/`. New `SAILOR_DIRS_RESOLVED_VIA_PACKAGE = ['styles']` skip mechanism in `cli/utils.js` parallels the components one but anchored at `src/lib/sailor/` instead of `src/lib/components/`. Two import sites in sailor's routes (`+layout.svelte` and `auth/+layout@.svelte`) rewritten. Tailwind `@source` directive simplified from the brittle `../../../../node_modules/sailorcms/src/lib` to plain `../..` — now that `sailor.css` lives in the package, that relative path resolves to `src/lib/` in both sailor's dev (`<repo>/src/lib`) and consumer (`<consumer>/node_modules/sailorcms/src/lib`), covering every migrated subtree from one source. Recipe for the remaining subtrees (`core/`, `utils/`, `remote/`, `i18n/`) is now mechanically identical: append the directory name to `SAILOR_DIRS_RESOLVED_VIA_PACKAGE`, bulk-rewrite `$sailor/<dir>/...` → `sailorcms/<dir>/...`.
