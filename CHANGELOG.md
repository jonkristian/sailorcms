...
This is a structural release: sailor's admin code now lives in `node_modules/sailorcms` and is resolved from the package, not copied into the consumer's tree. Consumer upgrades stop having to merge admin-code edits, and new admin files are picked up automatically.

### Added

- **Admin code now resolves from the `sailorcms` package** via wildcard subpath exports. Subtrees migrated this release: `components/sailor/` (64 files), `components/ui/` (240+ shadcn-svelte files), `composables/`, `core/` (auth, db, services, content, editor, files, hooks, settings, types, ui, utils — 117 files), `remote/`, `scripts/`, `assets/`, `styles/`, and `utils/`. The shadcn `cn()` helper moved from `sailor/utils.ts` (sibling) into `sailor/utils/shadcn.ts` (inside the migrated subtree). On every `core:update`, sailor skips writing these subtrees and prunes stale copies left from earlier versions. The consumer keeps only what's user-customizable: `templates/`, `generated/`, `i18n/messages/` and `i18n/paraglide/`, route shells, `hooks.server.ts`, and config files.

- **`npx sailor doctor`** — consumer-side setup healthcheck with `--fix`. Eight checks: stale admin imports in user-owned scaffold files (rewrites `$lib/components/...`, `$sailor/...`, `./src/lib/sailor/...` for migrated subtrees, including `drizzle.config.ts` at root); `sv create` scaffold clash with `(site)/` routes; `svelte.config.js` patches (`$sailor` + `sailorcms/*` aliases, `vitePreprocess({ script: true })`, runes, `experimental.async`, `kit.experimental.remoteFunctions`); `vite.config.ts` patches (tailwindcss + paraglide plugins, `resolve.dedupe`, `ssr.noExternal`, `optimizeDeps.exclude/include`); legacy `db:*` scripts in `package.json`; duplicate framework deps nested under `svelte/` and `sailorcms/` from bun's `file:` / `github:` install behavior; `sailor.sqlite` write-lock probe (diagnostic only). Read-only by default; `--fix` applies all auto-fixable failures. Exits non-zero on unresolved issues so it can be wired into CI. Colored output (auto-disables on non-TTY and respects `NO_COLOR`).

- **`npx sailor dev:sync-ui`** — maintainer-only command for normalizing `npx shadcn-svelte add <component>` output: rewrites `$lib/components/ui/...` cross-component references to `sailorcms/components/ui/...` and flattens bare barrels to `/index.js`. Establishes the `dev:*` namespace for any future maintainer-only commands.

### Changed

- **CLI command grouping** — `--help` output now groups commands by namespace: `core:*`, `db:*`, `search:*`, `users:*`, `doctor`, `dev:*`. Registration order in `cli/index.js` mirrors this with section comments.
- **Shared CLI helpers** — `loadConsumerEnv()` and `createConsumerLibsqlClient()` extracted in `cli/utils.js`; `db:repair`, `db:repair-timestamps`, `ensureDbDir`, `runMigrations`, and `createCliDbOrFail` all delegate. ~30 lines of inline dotenv + libsql boilerplate removed per repair command.
- **`dotenv` chatter suppressed** — `{ quiet: true }` everywhere. CLI no longer prefixes every invocation with `◇ injected env (X) from .env`.
- **`db:generate` description** clarified as the types-only subset of `db:update` (does not run drizzle-kit generate or migrations).

### Fixed

- **Redirects from sailor hooks were 500ing instead of 302ing.** Vite externalizes `node_modules` for SSR by default, so admin code resolved from the package evaluated `@sveltejs/kit` and `svelte` independently of the consumer's app code. Different `Redirect` class identities meant `throw redirect()` failed the framework's `instanceof` check, surfacing as 500 with a stringified-redirect body. `patchViteConfig` now injects `resolve.dedupe: ['@sveltejs/kit', 'svelte']` and `ssr.noExternal: ['sailorcms']`, plus `optimizeDeps.exclude: ['sailorcms']` (so esbuild's dep-scan doesn't try to resolve `$sailor/...` aliases inside package code) and `optimizeDeps.include: ['highlight.js/lib/core', 'highlight.js/lib/languages/json']` (so highlight.js's ESM exports condition takes effect).

- **`patchSvelteConfig` mirrors `sailorcms/*` paths into the consumer's `kit.alias`.** TypeScript's `moduleResolution: bundler` doesn't reliably probe `.ts` extensions in `node_modules` through wildcard exports without shipped `.d.ts` files. Mirroring all migrated subtree paths into kit.alias gives the consumer's tsconfig direct paths via SvelteKit's sync, so admin imports type-resolve cleanly. Sailor's own dev already had these aliases for self-reference.

- **`patchSvelteConfig` ensures `kit.experimental.remoteFunctions: true`.** Sailor uses SvelteKit remote functions for every admin RPC; the patcher previously handled the `$sailor` alias and `compilerOptions` but never touched `kit.experimental`, so fresh `core:init` left consumers seeing "To enable remote functions, add the following to your svelte.config.js" errors. Now finds the `kit: { ... }` block (matching nested braces correctly) and either splices `remoteFunctions: true` into an existing `experimental: { ... }` object or appends a new one.

- **`patchSvelteConfig` rewrites function-form `compilerOptions.runes` to flat `true`.** `sv create` scaffolds write a function form that returns runes mode for project files only and legacy mode for `node_modules`. Shipped Svelte 5 packages (`@lucide/svelte`, `bits-ui`) use runes (`$props()`) in their `.svelte` source, so compiling them in legacy mode breaks SSR with `<thing> is not defined`.

- **Bun's `file:` / `github:` installs nest sailor's full devDependencies tree** under `node_modules/sailorcms/node_modules/`. Once admin resolves from the package, sailor's imports walk up looking for `node_modules/<dep>` and find the nested copies first — same duplicate-instance symptom as the SSR externalization issue but caused by physical-path duplication. The CLI now strips the nested tree on every install (alongside the existing fix for `node_modules/svelte/node_modules/`); `npx sailor doctor --fix` catches the same condition for installs that didn't go through the CLI.

- **`cms-init` dep-mirror bug** — `cms-init.js` previously merged sailor's `dependencies` and `devDependencies`, then put everything in the consumer's `dependencies`. Result: `drizzle-kit` ended up in both sections (it's also force-added to `devDependencies` separately) and packages like `tailwindcss`, `tslib`, `terser`, `globals`, `rollup-plugin-visualizer`, `@inlang/paraglide-js`, `@tailwindcss/forms`, `@tailwindcss/typography` landed wrongly in `dependencies`. Now mirrors sailor's classification: deps go to deps, devDeps go to devDeps, with defensive deduping. `cms-update`'s dev-deps loop adds the same dedupe so existing consumers' duplicate `drizzle-kit` collapses on next update.

- **CLI loads admin code from the package, not the consumer's tree.** `db:generate` and `db:seed` previously imported `core/types.ts`, `core/db/adapter-factory.ts`, `core/utils/string.ts` from `process.cwd()/src/lib/sailor/core/...`. Once those paths moved into `node_modules/sailorcms`, the CLI broke. Now resolved relative to `cli/`'s own location (`pkgRoot = path.join(__dirname, '..', '..')`), so commands work in both sailor's own dev and any consumer.

- **`drizzle.config.ts` uses the package path.** Old scaffold imported `sailorDrizzleConfig` via `./src/lib/sailor/core/db/drizzle-config` — broke once `core/` migrated. Now imports from `sailorcms/core/db/drizzle-config`, which Node resolves via the package's exports map (or self-reference in sailor's own dev). Doctor's stale-imports check rewrites old scaffolds.

- **`cleanDir` preserves `i18n/paraglide/` across `core:update`.** The directory is generated by the paraglide vite plugin, not part of sailor's source — but cleanDir wiped it on every update, breaking `bun check` until the next dev/build run regenerated it. Now in the skip list alongside `templates/`, `generated/`, and `i18n/messages/`.
