...
- **Admin code resolves from the `sailorcms` package** via wildcard subpath exports. Subtrees migrated: `components/sailor/`, `components/ui/`, `composables/`, `core/`, `remote/`, `scripts/`, `assets/`, `styles/`, `utils/`. The shadcn `cn()` helper relocated to `utils/shadcn.ts` (inside the migrated subtree). On every `core:update`, sailor skips writing these subtrees and prunes stale copies. Consumer keeps only what's user-customizable: `templates/`, `generated/`, `i18n/`, route shells, `hooks.server.ts`, and config files. `patchSvelteConfig` and `patchViteConfig` now inject every consumer-side knob this requires (`sailorcms/*` aliases, `resolve.dedupe`, `ssr.noExternal`, `optimizeDeps.exclude/include`); `cms-init`/`cms-update` strip duplicate framework deps that bun's `file:`/`github:` installs nest under `node_modules/sailorcms/node_modules/`.

- **`npx sailor doctor`** — consumer-side healthcheck with `--fix`. Eight checks covering stale admin imports in user-owned scaffold files (incl. `drizzle.config.ts` at root), scaffold-route clashes, `svelte.config.js` and `vite.config.ts` patches, legacy `db:*` scripts, duplicate framework deps nested under `svelte/` or `sailorcms/`, and `sailor.sqlite` write-lock probe. Read-only by default; `--fix` applies all auto-fixable failures. Colored output (respects `NO_COLOR` and non-TTY). Exits non-zero on unresolved issues so it can be wired into CI.

- **`npx sailor dev:sync-ui`** — maintainer-only command for normalizing `npx shadcn-svelte add <component>` output (rewrites `$lib/components/ui/...` → `sailorcms/components/ui/...`, flattens bare barrels). Establishes the `dev:*` namespace for any future maintainer-only commands.

### Changed

- **CLI cleanup** — commands grouped by namespace in `--help` (`core:*`, `db:*`, `search:*`, `users:*`, `doctor`, `dev:*`); shared `loadConsumerEnv()` and `createConsumerLibsqlClient()` helpers extracted; `dotenv` chatter suppressed (no more `◇ injected env` banner per invocation); `db:generate` description clarified as the types-only subset of `db:update`.

### Fixed

- **`cms-init` dep-mirror bug** — `cms-init.js` previously merged sailor's `dependencies` and `devDependencies`, then put everything in the consumer's `dependencies`. Result: `drizzle-kit` ended up in both sections, and `tailwindcss`, `tslib`, `terser`, `globals`, `rollup-plugin-visualizer`, `@inlang/paraglide-js`, `@tailwindcss/forms`, `@tailwindcss/typography` landed wrongly in `dependencies`. Now mirrors sailor's classification: deps to deps, devDeps to devDeps, with defensive deduping. `cms-update`'s dev-deps loop adds the same dedupe so existing consumers' duplicate `drizzle-kit` collapses on next update.
