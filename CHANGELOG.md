...
- **Admin code now resolves from the `sailorcms` package instead of being copied into the consumer's tree.** New wildcard subpath exports (`sailorcms/components/sailor/*`, `sailorcms/core/*`, `sailorcms/remote/*`, `sailorcms/utils/*`, `sailorcms/styles/*`) let consumers resolve admin code from `node_modules/sailorcms` rather than from copied files under `src/lib/`. Three subtrees migrated this release: all 64 admin components, the admin stylesheet, and the entire core module (auth, db, services, content, editor, files, hooks, settings, types, ui, utils — 117 files). On every `core:update`, sailor skips writing those subtrees and prunes stale copies left from earlier versions. Consumer upgrades no longer have to merge admin-code edits, and new admin files are picked up automatically. Templates, generated schema, route shells, `hooks.server.ts`, and config files still live in the consumer — anything user-customizable.

- **`npx sailor doctor` — consumer-side setup healthcheck.** Diagnoses the class of issues that previously broke `bun dev` or `bun run build` after a sailor upgrade. Checks: stale admin imports left behind in user-owned scaffold files; top-level `+page.svelte` / `+layout.svelte` from `sv create` colliding with sailor's `(site)/` routes; missing `svelte.config.js` patches (`$sailor` alias, `vitePreprocess({ script: true })`, runes, `experimental.async`, `kit.experimental.remoteFunctions`); missing `vite.config.ts` patches (tailwindcss + paraglide plugins, `resolve.dedupe`); legacy `db:*` scripts in `package.json`; bun's nested-`node_modules` duplicates under `svelte/` and `sailorcms/`; and a `sailor.sqlite` write-lock probe (diagnostic only — the fix is to stop your dev server). Default invocation is read-only; `--fix` applies all auto-fixable failures. Exits non-zero on unresolved issues so it can be wired into CI.

### Fixed

- **`patchSvelteConfig` ensures `kit.experimental.remoteFunctions: true`.** Sailor uses SvelteKit remote functions for every admin RPC; without the flag vite errors with "To enable remote functions, add the following to your svelte.config.js" at transform time. The patcher previously handled the `$sailor` alias and `compilerOptions` but never touched `kit.experimental`, so fresh `core:init` left consumers having to add it themselves. Now finds the `kit: { ... }` block (matching nested braces correctly) and either splices `remoteFunctions: true` into an existing `experimental: { ... }` object or appends a new one.

- **`patchSvelteConfig` rewrites function-form `compilerOptions.runes` to flat `true`.** `sv create` scaffolds write a function form that returns runes mode for project files only and legacy mode for `node_modules`. Shipped Svelte 5 packages (`@lucide/svelte`, `bits-ui`) use runes (`$props()`) in their `.svelte` source, so compiling them in legacy mode breaks SSR with `<thing> is not defined`. The previous patcher only fired when `compilerOptions:` was absent entirely; now it detects a non-`true` runes value and rewrites the line.

- **`patchViteConfig` adds `resolve.dedupe: ['@sveltejs/kit', 'svelte']`.** Vite externalizes `node_modules` for SSR by default, so admin code resolved from `node_modules/sailorcms` evaluates `@sveltejs/kit` and `svelte` independently of the consumer's app code. Different `Redirect` class identities mean a `throw redirect()` from a sailor hook fails the framework's `instanceof` check — consumers see 500s with stringified-redirect bodies instead of 302s. `dedupe` forces a single instance across both module graphs.

- **Bun's `file:` / `github:` installs nest sailor's full devDependencies under `node_modules/sailorcms/node_modules/`.** Once admin code resolves from the package, sailor's imports walk up looking for `node_modules/<dep>` and find the nested copies first — same duplicate-instance symptom as the SSR externalization issue but caused by physical-path duplication rather than module-graph duplication. The CLI now strips the nested tree on every install (alongside the existing fix for `node_modules/svelte/node_modules/`). `npx sailor doctor --fix` catches the same condition for installs that didn't go through the CLI.
