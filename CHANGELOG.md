...
- **Generated TS type names now derive from `slug` (stable), with singularization on collections/globals and kind-suffixes on globals/blocks (breaking change — search-and-replace your imports).** The type generator was emitting interface + union names from each template's _translatable_ display name, so localizing `name.singular: 'Hero Section'` → `'Hero Seksjon'` silently renamed every consumer-side TS import (`HeroSection` → `HeroSeksjon`). New rules: collection / global type names are `pluralize.singular(slug)` PascalCased (matches Payload's convention, gives natural singular row-type names like `Post`/`Page`/`Menu` instead of `Posts`/`Pages`/`Menus`); blocks PascalCase the slug as-is (block slugs are typically already singular). To prevent cross-kind collisions (e.g. a `globals/employees.ts` and `blocks/employees.ts` both PascalCasing to `Employee`) and to keep sailor's built-in `Settings` interface unambiguous when a consumer defines a `settings` global, **globals are suffixed with `Global` and blocks with `Block`**; collections stay un-suffixed. New optional `typeName?: string` field on `CollectionDefinition` / `GlobalDefinition` / `BlockDefinition` overrides the auto-derived name (use for collective nouns like `settings` → `Setting` reads weird, acronyms like `faq` → `Faq` loses casing, or any custom name). Concrete shifts on stock templates: `Post` / `Page` unchanged, `Menu` → `MenuGlobal`, `Category` → `CategoryGlobal`, `Submission` → `SubmissionGlobal`, `FAQ` → `FaqGlobal` (declare `typeName: 'FAQGlobal'` to preserve casing), `Detail` → `DetailGlobal`, `HeroSection` → `HeroBlock`, `FeaturesSection` → `FeaturesBlock`, `MediaTextSection` → `MediaTextBlock`, `RichText` → `RichTextBlock`, `Gallery` → `GalleryBlock`. Run `npx sailor db:update` to regenerate, then update every `import type { ... } from '$sailor/generated/types'` referencing a renamed type. The `CollectionTypes` / `GlobalTypes` / `BlockTypes` union types still exist; members shift accordingly. Adds `pluralize` as a dep (~10KB, CLI-side only — handles irregular plurals like `Categories→Category`, `People→Person`, `Children→Child`).
